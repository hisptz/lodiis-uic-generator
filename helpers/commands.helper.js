...
async function getVerifiedCommands(parameters) {
  ...
  if (parameters && parameters.length) {
    const actionParam =
      parameters[actionIndex] && parameters[actionIndex].toLocaleLowerCase()
        ...
        : null;
    const action =
      actionParam && actions[actionParam] && actions[actionParam].name
        ? actions[actionParam].name
        : '';
    ...
    switch (action) {
      case generate.name:
        ...
        from =
          actionTypeIndexes && parameters[actionTypeIndexes.startDateIndex]
            ? getGenerateDate(
                parameters,
        ...
        to =
          actionTypeIndexes && parameters[actionTypeIndexes.endDateIndex]
            ? getGenerateDate(
                parameters,
        ...
        await logsHelper.addLogs(
          'INFO',
          `COMMAND: update from ${from} to ${to}`
        );
        if (
          from === 'ERROR' ||
          to === 'ERROR' ||
      ...
      case update.name:
        ...
        statusOption = actionTypeIndexes
          ? getUpdateCommand(
              parameters,
              actionTypeIndexes.statusIndex,
              actionTypeIndexes.statusOptionIndex
            )
          : '';
        await logsHelper.addLogs(
          'INFO',
          `COMMAND: update status ${statusOption}`
        );
        break;
      default:
        console.log(
          'There is an error in a command, please review it and try again later'
        );
        break;
    }
...
function getGenerateDate(parameters, dateTypeIndex, dateIndex, type) {
  ...
  if (
    ...
    dateParam =
      parameters[dateIndex] &&
      moment(parameters[dateIndex], 'YYYY-MM-DD').isValid()
